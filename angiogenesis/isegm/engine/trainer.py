...
class ISTrainer(object):
    ...
    def run(self, num_epochs, start_epoch=None, validation=True):
        ...
        for epoch in range(start_epoch, num_epochs):
            total_train_loss = self.training(epoch)
            if validation:
                total_val_loss = self.validation(epoch)
    ...
    def training(self, epoch):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            if self.is_master:
                ...
                #     )
                # global_step = 1
                # if self.image_dump_interval > 0 and epoch % global_step == 0:
                #     self.save_visualization(splitted_batch_data, outputs, epoch, global_step, prefix='train')
        ...
        info_loss =f'@==Final== Epoch[{epoch}]  Training Loss: {train_loss/len(tbar):.4f}'
    ...
    def validation(self, epoch):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            loss, batch_losses_log, splitted_batch_data, outputs = self.batch_forward(batch_data, validation=True)
            ...
            if self.is_master:
                ...
                global_step = 1
                if self.image_dump_interval > 0 and epoch % global_step == 0:
                    self.save_visualization(splitted_batch_data, outputs, epoch, global_step, prefix='val')
        if self.is_master:
            ...
            for metric in self.val_metrics:
                self.sw.add_scalar(tag=f'{log_prefix}Metrics/{metric.name}',
                                   value=metric.get_epoch_value(),
                                   global_step=epoch, disable_avg=True)
        info_loss =f'@==Final== Epoch[{epoch}]  Validation Loss: {val_loss/len(tbar):.4f}'
        logger.info(info_loss)
        return val_loss
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            image_feats, external_feats = self.net.get_image_feats(image)
            ...
            with torch.no_grad():
                ...
                for click_indx in range(num_iters):
                    ...
                    #points = (points[:,:,], image_record["point_labels"])
                    prompt_feats = self.net.get_prompt_feats(visual_prompts)
                    prev_mask = torch.sigmoid(self.net(image_feats,prompt_feats, external_feats)['instances'])
            ...
            prompt_feats = self.net.get_prompt_feats(prompts)
            #output = self.net(image.shape, image_feats, prompt_feats)
            output = self.net(image_feats,prompt_feats, external_feats)
    ...
    def save_visualization(self, splitted_batch_data, outputs, epoch, global_step, prefix):
        output_images_path = self.cfg.VIS_PATH / prefix
        if self.task_prefix:
            output_images_path /= self.task_prefix
        ...
        if not output_images_path.exists():
            output_images_path.mkdir(parents=True)
        image_name_prefix = str(epoch) + "_" + f'{global_step:06d}'
...
def get_next_points(pred, gt, points, click_indx, pred_thresh=0.49):
    ...
    for bindx in range(fn_mask.shape[0]):
        ...
        # 伪代码示例：基于距离的概率计算
        fn_weight = fn_max_dist / (fn_max_dist + fp_max_dist)
        #is_positive = fn_max_dist > fp_max_dist
        if fn_weight in [0, 1]:
            # 定义概率权重
            probabilities = [fn_weight, 1-fn_weight]

            # 生成布尔结果
            is_positive = np.random.choice([True, False], p=probabilities)
        else:
            is_positive = fn_max_dist > fp_max_dist
        dt = fn_mask_dt if is_positive else fp_mask_dt
        inner_mask = dt > max(fn_max_dist, fp_max_dist) / 2.0
