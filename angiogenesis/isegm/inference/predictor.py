...
class BasePredictor(object):
    ...
    def set_image(self, image: np.ndarray) -> None:
        ...
        self.image_feats, self.external_feature = self.model.get_image_feats(image)
    ...
    def predict(self, clicker: Clicker) -> np.ndarray:
        ...
        # click1_points = points_nd[:,:,:2]
        # click1_labels = points_nd[:,:,2].clone()
        # mask_100 = (click1_labels == 100)
        # mask_others = (click1_labels != -1) & (click1_labels != 100)

        # # 先处理非 -1 和非 100 的数值，设为 0
        # click1_labels[mask_others] = 0

        # # 再处理值为 100 的位置，设为 1
        # click1_labels[mask_100] = 1
        # click1 = (click1_points, click1_labels)

        # prompt_feats = self.model.get_prompt_feats(self.orig_img_shape, prompts)
        import time
        start = time.time()
        prompt_feats = self.model.get_prompt_feats(prompts)
        ...
        end1 = time.time()
        print("get_prompt_feats time:", end - start)
        print("get_visual_prompts_feats time:", end1 - end)
