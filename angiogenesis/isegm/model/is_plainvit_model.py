...
from isegm.model.modeling.cross_attention import CrossBlock,Block_mask
...
class PlainVitModel(nn.Module):
    ...
    def __init__(
        ...
        if self.fusion_type == 'naive':
            ...
            self.fusion_blocks = nn.Sequential(
                *[Block_mask(**fusion_params['params']) for _ in range(depth)]
            )
        elif self.fusion_type == 'mask_attention':
            depth = int(fusion_params['depth'])
            self.fusion_blocks = nn.Sequential(
    ...
    def get_prompt_feats(self, inputs,keep_shape=True):
    ...
    def forward(self, image_feats, prompt_feats, addition_feats = None):
    ...
    #     return self.get_prompt_feats(prompts)
