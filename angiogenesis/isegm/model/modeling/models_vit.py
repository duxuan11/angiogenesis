...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224,224), patch_size=(16, 16), in_chans=3,
                 ...
                 proj_drop_rate=0., out_chans: int = 256, norm_layer=None, act_layer=None):
        super().__init__()
    ...
    def forward(self, x, other_feats=None, keep_shape=False):
        ...
        if self.global_atten_freq <= 1:
            ...
            for i in range(1, num_blocks + 1):
                ...
                x = self.blocks[i-1](x)
                if i==1:
                    y = self.window_split_reverse(x, x_size=(H, W))
                    # save the first block output for external features
                    C_new = y.shape[-1]
                    H_new = H // self.patch_embed.patch_size[0]
                    W_new = W // self.patch_embed.patch_size[1]
                    y = y.transpose(1,2).contiguous().reshape(B, C_new, H_new, W_new)
                    external_features.append(y)
...
if __name__ =="__main__":
    ...
    from ..is_plainvit_model import SimpleFPN, SegmentationHead
    start = time()
    input_image = torch.randn(1,3,  1024, 1024)  # 输入图像 (H, W) = 512x512
    model = vit_xtiny_patch16(img_size=(1024,1024))  # 输出形状 (1, 3072, 128, 128)  # 假设 patch_size=4
    model2 = SimpleFPN()
    model3 = SegmentationHead()
