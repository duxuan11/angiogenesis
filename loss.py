...
class PixLoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.criterions_last = {}
        if 'NFL' in self.lambdas_pix_last and self.lambdas_pix_last['NFL']:
            self.criterions_last['NFL'] = NormalizedFocalLossSigmoid() if not self.config.use_fp16 else nn.BCEWithLogitsLoss()
